...
class AdvancedMarketAnalyzer:
    def __init__(self,
        ...
        # Минимальные требования к условиям
        self.min_requirements = {
            'volume_ratio': 2.0,  # Объем выше среднего в 2 раза
            'depth_ratio': 5.0,  # Глубина стакана в 5 раз больше объема
            'bid_ask_imbalance': 1.5,  # Дисбаланс бидов к аскам
            'tick_size_ratio': 0.1,  # Размер тика к спреду
            'min_btc_correlation': 0.3,  # Минимальная корреляция с BTC
            'min_relative_strength': 1.2  # Минимальная относительная сила
        }
    ...
    async def analyze_microstructure(self, symbol: str, market_data: MarketData) -> Dict:
        ...
        try:
            ...
            small_buffer = self.buffers['small'][symbol]
            ...
            try:
                trades = await asyncio.to_thread(self.exchange.fetch_trades, symbol, limit=100)
            ...
            try:
                components = {
                    ...
                    'clustering': min(1.0, clusters.get('imbalance', 1.0) if clusters.get('imbalance',
                                                                                          1.0) < 2 else 2 - clusters.get(
                        'imbalance', 1.0)),
            ...
            try:
                ...
                logging.warning(f"Ошибка при расчете итогового скора {symbol}: {e}")
                return {'score': 0, 'suitable': False}
    ...
    async def analyze_market_conditions(self,
        ...
        depth_ratio = market_data.market_depth / market_data.volume
    ...
    async def fetch_market_data(self, symbol: str) -> Optional[MarketData]:
        ...
        try:
            ...
            orderbook_task = asyncio.create_task(
                asyncio.to_thread(self.exchange.fetch_order_book, symbol)
            )
            trades_task = asyncio.create_task(
                asyncio.to_thread(self.exchange.fetch_trades, symbol, limit=100)
            )
    ...
    async def run_realtime_analysis(self):
        ...
        while True:
            try:
                ...
                # Инициализация буферов для новых символов
                ...
                for symbol in symbols:
                    ...
                    try:
                        ...
                        if analysis['suitable']:
                            logging.info(f"\nНайдена перспективная возможность для {symbol}:")
                            logging.info(f"Вероятность: {analysis['probability']:.2%}")
                            logging.info("Компоненты анализа:")
                            for component, value in analysis['components'].items():
                                logging.info(f"- {component}: {value:.3f}")
                            logging.info(f"Микроструктурный анализ: {analysis['micro_analysis']}")
                            ...
                            if len(self.active_positions) < self.max_positions:
                                ...
                                logging.info(f"Открыта новая позиция по {symbol}")
                                logging.info(f"Цена входа: {position['entry_price']}")
                                logging.info(f"Целевая цена: {position['target_price']}")
...
if __name__ == "__main__":
    try:
        analyzer = AdvancedMarketAnalyzer(
            ...
            min_probability=0.20,  # Минимальная вероятность для входа
        ...
        asyncio.run(analyzer.run_realtime_analysis())
